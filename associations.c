...
{
    return pearson_hash(key, chars_length(key)) % as->table->length;
}
...
void associations_remove(struct associations *as, char *key)
{
    struct association *a;
    struct nodes *n;
    int i, index;
